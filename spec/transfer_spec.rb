...
RSpec.describe Transfer do
  describe "#execute" do
    ...
    context "when the amount is negative" do
      let(:transfer) { Transfer.new(from: "1111234522226789", to: "1212343433335665", amount: BigDecimal("-10.00")) }

      it "doesn't raise and doesn't move any money" do
        expect { transfer.execute(ledger) }.not_to raise_error
        expect(account_one.balance).to eq(BigDecimal("1000.00"))
        expect(account_two.balance).to eq(BigDecimal("500.00"))
      end
      it "returns an unsuccessful TransferResult object" do
        result = transfer.execute(ledger)
        expect(result.success?).to eq(false)
        expect(result.reason).to eq(TransferResult::INVALID_AMOUNT)
      end
    end

    context "when the account looked up doesn't exist" do
      let(:no_from_account_transfer) do
        Transfer.new(from: "1111234522226787", to: "1212343433335665", amount: BigDecimal("2000.00"))
      end
      let(:no_to_account_transfer) do
        Transfer.new(from: "1111234522226789", to: "1212343433335663", amount: BigDecimal("2000.00"))
      end
