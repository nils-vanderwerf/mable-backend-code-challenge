...
RSpec.describe Account do
  ...
  describe "#initialize" do
    it "successfully creates the account" do
      account = Account.new(number: "1111234522226789", balance: BigDecimal("100.00"))
  ...
  describe "#debit!" do
    ...
    context "when an amount attempts to be debited from the account and the remaining amount is negative" do
      it "raises an error" do
        ...
        expected_message = "Can't debit amount of #{amount.to_s('F')} for this account because the balance is " \
                           "currently 100.0 and would become negative"
        expect do
          account.debit!(amount)
        end.to raise_error(Account::InsufficientFundsError, /^#{Regexp.escape(expected_message)}/)
